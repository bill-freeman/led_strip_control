...
void loop() {
  ...
  // see if the string is the right size
  if(req.length() == 13)
  {
    // message is 255 255 255 0
    //             R   G   B  S
    r = req.substring(0,3).toInt();
    g = req.substring(4,7).toInt();
    b = req.substring(8,11).toInt();
    sw = req.substring(12).toInt();
